...
def main():
    ...
    file_list, label_list, label_cn_list = f"{args.lrs3}/file.list", f"{args.lrs3}/label.list", f"{args.lrs3}/label_cn.list"
    ...
    assert os.path.isfile(label_cn_list) , f"{label_cn_list} not exist -> run lrs3_prepare.py first"
    ...
    with NamedTemporaryFile(mode="w") as f:
        ...
        label_cn_text = [ln.strip() for ln in open(label_cn_list).readlines()]
        label_text.extend(label_cn_text)
        for t in label_text:
            f.write(t.lower() + "\n")
    ...
    def setup_target(target_dir, train, valid, test):
        for name, data in zip(['train', 'valid', 'test'], [train, valid, test]):
            with open(f"{target_dir}/{name}.tsv", 'w') as fo:
                fo.write('/\n')
                for fid, _, _, nf_audio, nf_video in data:
            ...
            with open(f"{target_dir}/{name}.wrd", 'w') as fo:
                for _, label, label_cn, _, _ in data:
                    fo.write(f"{label}\n")
            with open(f"{target_dir}/{name}_cn.wrd", 'w') as fo:
                for _, _, label_cn, _, _ in data:
                    fo.write(f"{label_cn}\n")
        shutil.copyfile(vocab_path, f"{target_dir}/dict.wrd.txt")
        return

    fids, labels, labels_cn = [x.strip() for x in open(file_list).readlines()], [x.strip().lower() for x in open(label_list).readlines()], [x.strip().lower() for x in open(label_cn_list).readlines()]
    ...
    for fid, label, label_cn, nf_audio, nf_video in zip(fids, labels, labels_cn, nfs_audio, nfs_video):
        part = fid.split('/')[0]
        # print(part)
        if part == 'test':
            test.append([fid, label, label_cn, nf_audio, nf_video])
        else:
            if fid in valid_fids:
                valid.append([fid, label, label_cn, nf_audio, nf_video])
            else:
                train_all.append([fid, label, label_cn, nf_audio, nf_video])
                if part == 'trainval':
                    train_sub.append([fid, label, label_cn, nf_audio, nf_video])
