...
class AVTNetEncoder(FairseqEncoder):
    def __init__(self, cfg, tgt_dict=None):
        ...
        self.av_layernorm = LayerNorm(cfg.decoder_embed_dim)
...
@register_model("avtnet_transformer_seq2seq", dataclass=AVTNetSeq2SeqConfig)
class AVTNetTransformerSeq2Seq(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder, tgt_dict, cfg):
        ...
        self.lm_transformer = TransformerEncoder(lm_args,tgt_dict,lm_encoder_embed_tokens)
        self.lm_layernorm = LayerNorm(cfg.decoder_embed_dim)
        print('--------------------not load transformer-------------------------')
        # state_dict = torch.load(cfg.lm_path, map_location=torch.device("cpu"))
        # lm_state_dict = state_dict['model']
        # new_lm_state_dict = {}
        # for key in lm_state_dict.keys():
        #     if key.startswith('encoder.'):
        #         new_key = key.replace('encoder.', '')
        #         new_lm_state_dict[new_key] = lm_state_dict[key]
        # self.lm_transformer.load_state_dict(new_lm_state_dict)
    ...
    def forward(self, **kwargs):
        ...
        # with torch.no_grad():
        lm_encoder_out = self.lm_transformer(src_tokens=kwargs["target"],src_lengths=kwargs["target_lengths"])
        ...
        lm_encoder_out["encoder_out"] = self.lm_layernorm(lm_encoder_out["encoder_out"])
