...
if DBG:
    from avtnet.datasets.avtnet_dataset import AVTNetDataset
...
@dataclass
class AVTNetModelConfig(FairseqDataclass):
    data: str = field(
...
@register_task("avtnet_task", dataclass=AVTNetModelConfig)
class AVTNetTask(FairseqTask):

    cfg: AVTNetModelConfig
    ...
    def load_dictionaries(self):
        ...
        dictionaries[0].add_symbol("</st_s>")
    ...
    @classmethod
    def setup_task(
        cls, cfg: AVTNetModelConfig, **kwargs
    ) -> "AVTNetModelConfig":
        if cfg.pdb:
            import pdb
            pdb.set_trace()
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        st_paths = [
            f"{self.get_label_dir()}/{split}_cn.{l}" for l in self.cfg.labels
        ]
        ...
        self.datasets[split] = AVTNetDataset(
            manifest,
            sample_rate=self.cfg.sample_rate,
            label_paths=paths,
            label_st_paths = st_paths,
    ...
    def build_generator(
        self, models, args, eos=None, seq_gen_cls=None, extra_gen_cls_kwargs=None, prefix_allowed_tokens_fn=None,
    ):
        """
