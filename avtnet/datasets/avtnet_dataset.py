    ...
    with open(manifest_path) as f:
        ...
        for ind, line in enumerate(f):
            ...
            if min_keep is not None and sz < min_keep:
                ...
                video_path = os.path.splitext(items[1])[0]+'.npy'
                audio_path = os.path.splitext(items[2])[0]+'.npy'
                # video_path = items[1]
                # audio_path = items[2]
                audio_id = items[0]
...
def load_st_label_offset(label_path, inds, tot):
...
class AVTNetDataset(FairseqDataset):
    def __init__(
            self,
            manifest_path: str,
            sample_rate: float,
            label_paths: List[str],
            label_st_paths: List[str],
        ...
        self.num_st_labels = len(label_st_paths)
        ...
        if store_labels:
            ...
            self.label_st_list = [load_label(p, inds, tot) for p in label_st_paths]
        else:
            self.label_paths = label_paths
            self.label_st_paths = label_st_paths
            self.label_offsets_list = [
                load_label_offset(p, inds, tot) for p in label_paths
            ]
            self.label_st_offsets_list = [
                load_st_label_offset(cp, inds, tot) for cp in label_st_paths
            ]
        assert (
    ...
    def get_st_label(self, index, label_idx):
        if self.store_labels:
            label = self.label_st_list[label_idx][index]
        else:
            with open(self.label_st_paths[label_idx], 'rb') as f:
                offset_s, offset_e = self.label_st_offsets_list[label_idx][index]
                f.seek(offset_s)
                label = f.read(offset_e - offset_s)
    ...
    def get_st_labels(self, index):
        return [self.get_st_label(index, i) for i in range(self.num_st_labels)]
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            wav_data= np.load(audio_fn)
            assert len(wav_data.shape) == 1
            if np.random.rand() < self.noise_prob:
                wav_data = self.add_noise(wav_data)
            audio_feats = logfbank(wav_data, samplerate=16_000).astype(np.float32) # [T, F]
    ...
    def __getitem__(self, index):
        ...
        st_labels = self.get_st_labels(index)
        fid = self.names[index][1].split(':')[1]
        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels, "label_st_list": st_labels}
    ...
    def collater(self, samples):
        ...
        targets_st_by_label = [
            [s["label_st_list"][i] for s in samples]
            for i in range(self.num_labels)
        ]
        targets_list, lengths_list, ntokens_list = self.collater_label(
            targets_by_label, audio_size, audio_starts
        )
        targets_st_list, lengths_st_list, ntokens_st_list = self.collater_label(
            targets_st_by_label, audio_size, audio_starts, self.label_processors[0].dictionary.index("</st_s>")
        )
        ...
        if self.single_target:
            ...
            batch["target_st_lengths"] = lengths_st_list[0]
            batch["ntokens"] = ntokens_list[0]
            batch["ntokens_st"] = ntokens_st_list[0]
            if self.is_s2s:
                batch['target'],batch['target_st'], net_input['prev_output_tokens'], net_input['prev_st_output_tokens']  = targets_list[0][0], targets_st_list[0][0], targets_list[0][1], targets_st_list[0][1]
                net_input['target'], net_input['target_st'] = targets_list[0][0],targets_st_list[0][0]
                net_input['target_lengths'] = lengths_list[0]
            else:
    ...
    def collater_seq_label_s2s(self, targets, pad, eos):
        lengths = torch.LongTensor([len(t) for t in targets])
        ntokens = lengths.sum().item()
        pad, eos = self.label_processors[0].dictionary.pad(), self.label_processors[0].dictionary.eos() if eos is None else eos
    ...
    def collater_label(self, targets_by_label, audio_size, audio_starts, eos=None):
        targets_list, lengths_list, ntokens_list = [], [], []
        itr = zip(targets_by_label, self.label_rates, self.pad_list)
        for targets, label_rate, pad in itr:
            if label_rate == -1:
                if self.is_s2s:
                    targets, lengths, ntokens = self.collater_seq_label_s2s(targets, pad, eos)
                else:
