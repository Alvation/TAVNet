...
class AVTNetTransformerCriterion(LabelSmoothedCrossEntropyCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, null_loss = self.compute_loss(model, net_output[0], sample, reduce=reduce)
        lm_st_loss, lm_st_null_loss = self.compute_st_loss(model, net_output[1], sample, reduce=reduce)
        loss = loss + lm_st_loss
        sample_size = (
        ...
        if self.report_accuracy:
            n_correct_lm, total_lm = self.compute_accuracy(model, net_output[0], sample)
            n_correct_lm_st, total_lm_st = self.compute_st_accuracy(model, net_output[1], sample)
            logging_output["n_correct"] = utils.item(n_correct_lm.data)
            logging_output["total"] = utils.item(total_lm.data)
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_derived(
            "ppl_st", lambda meters: utils.get_perplexity(meters["lm_st_null_loss"].avg)
        )
        ...
        if total_lm_st > 0:
            metrics.log_scalar("total_lm_st", total_lm_st)
