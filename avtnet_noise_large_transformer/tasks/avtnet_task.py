...
if DBG:
    from avtnet_noise_large_transformer.datasets.avtnet_dataset import AVTNetDataset
...
@dataclass
class AVTNetTransformerConfig(FairseqDataclass):
    data: str = field(
...
@register_task("avtnet_transformer_task", dataclass=AVTNetTransformerConfig)
class AVTNetTransformerTask(FairseqTask):

    cfg: AVTNetTransformerConfig
    ...
    @classmethod
    def setup_task(
        cls, cfg: AVTNetTransformerConfig, **kwargs
    ) -> "AVTNetTransformerConfig":
        if cfg.pdb:
            import pdb
            pdb.set_trace()
        return cls(cfg)
