...
if DBG:
    from avtnet_noise_large_transformer.datasets.t_dataset import AVTNetDataset
...
@dataclass
class TNetTransformerConfig(FairseqDataclass):
    data: str = field(
...
@register_task("t_task", dataclass=TNetTransformerConfig)
class TNetTransformerTask(FairseqTask):

    cfg: TNetTransformerConfig
    ...
    @classmethod
    def setup_task(
        cls, cfg: TNetTransformerConfig, **kwargs
    ) -> "TNetTransformerConfig":
        if cfg.pdb:
            import pdb
            pdb.set_trace()
        return cls(cfg)
